...
class Summarizer():
    def __init__(self, w2v_path=str(Path(__file__).absolute().parent).replace('\\','/')+"/data/vi.vec"):
        self.clearner = Preprocessor()
        self.vectorizer = Vectorizer(w2v_path)

    def summarize(self, paragraph, mode="clustering", keep_sentences=3):
        origin_sentence = sent_tokenize(paragraph)
        sentences = self.clearner.preprocessing(paragraph)
        sent_vectors = self.vectorizer.vectorize(sentences, mode="count")  # row vector
